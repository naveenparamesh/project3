...
int main(){
	...
	// this matrix holds TF of all the documents' words
	...
	map<string, int> numOccurWordInDoc; //holds number of times word appears in doc
	double numWordsInDoc = 0; // number of times words in the doc
	string cleaned_word = "";

	//basically we loop through all the words from all the document, and then
	// we calculate the term frequency of each of the words, across ALL documents.
	// then we simply set these TFs into the matrix at the corresponding spot.
	...
	//picks out all the words from the query
	...
	//basically we loop through all the cleaned query words and calculate its
	// term frequency and simply set it into the quertTF vector at the 
	// corresponding index.
	...
	//so here we basically calculate the distances of each of the documents to the query
	// and store them in a vector, whose type is a struct document_infos, that holds
	// the document number and the document distance. This will help us later after
	// we sort the distances, so we can know which document actually relates to what
	// distance, so we ultimately know which document to return
	...
	//this repeats the phase all over again if the nearest document is below the 
	// specified threshold. All we do is add the new subtypes of each word from the 
	// query into the matrix and query vectors and repeat the process all over again
	// except we start at the index where the old vectors stopped at so we don't go
	// through the vectors analyzing words we already calculated TFs for.
	if(document_infos.at(0).docDistance < threshold){
		...
		if(document_infos.at(0).docDistance < threshold){
			cout << "no results for this query..." << endl; // basically if its still lower, then say not found
		}
		else {
			cout << "your query is most likely located at: " << endl;
			for(int i = 0; i < 6; i++){ //display top 6
